  ...
  write: function(stream, callback) {
    ...
    if (this.value instanceof File) {
		...
		fs.open(this.value.path, "r", (err, fd)=>{
			if (err) throw err;
			var position = 0;
			var readBuffer = new Buffer(1024 * 4);

            const reader = ()=>{
                var chunk = fs.readFileSync(this.value.path, 'binary')
                // chunk = chunk.replace('foo', 'bar')
                // fs.writeFileSync(this.value.path, chunk, 'binary')
			...
			// (function reader () {
             //    fs.read(fd, readBuffer, position, "binary", function (er, chunk) {
			// 		if (er) callback(err);
			// 		stream.write(chunk);
			// 		position += 1024 * 4;
			// 		if (chunk) reader();
